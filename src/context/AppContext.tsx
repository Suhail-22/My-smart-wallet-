...
export const AppProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
        ...
        if (Notification.permission === 'granted') {
          try {
            new Notification("تذكير يومي 📝", {
                body: "لم تقم بتسجيل أي مصروفات اليوم. هل نسيت شيئاً؟",
                icon: "/vite.svg" 
            });
    ...
    // Run check once on mount (with a small delay to ensure data loaded)
    const timer = setTimeout(checkDailyLog, 5000); 
    ...
    const checkRecurring = () => {
        ...
        currentTxList.forEach(t => {
            if (t.isRecurring) {
                ...
                // If the day of month has passed or is today
