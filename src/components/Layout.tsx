...
import { 
  ...
  HelpCircle,
  LogOut,
  Users // <-- مطلوب لشريط التنقل السفلي
} from 'lucide-react';
...
const Layout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const menuItems = [
    ...
    { path: '/transactions', label: 'المعاملات', icon: Users },
  ...
  return (
    <div className={`flex flex-col h-screen max-w-lg mx-auto ${theme === 'dark' ? 'dark' : ''}`}>
      ...
      <header className='fixed top-0 left-0 right-0 max-w-lg mx-auto bg-white dark:bg-dark-800 shadow-md z-40 p-4 flex items-center justify-between'>
        {/* زر الإعدادات (الترس) - تم استبدال زر القائمة */}
      ...
      {/* الشريط الجانبي */}
      <aside
        className={`fixed inset-y-0 right-0 z-50 w-64 bg-white dark:bg-dark-900 shadow-xl transform transition-transform duration-300 ${isSidebarOpen ? 'translate-x-0' : 'translate-x-full'} md:hidden`}
      >
      ...
      <div className="fixed bottom-0 left-0 right-0 max-w-lg mx-auto bg-white dark:bg-dark-900 shadow-xl z-50">
        <div className="flex justify-around items-center p-2">
          ...
          <button
            ...
            className="bg-primary-500 text-white rounded-full w-14 h-14 flex items-center justify-center shadow-2xl -mt-6"
          >
      ...
      {/* نافذة نموذج المعاملة المنبثقة */}
      {isTransactionFormOpen && (
        <div className="fixed inset-0 bg-black/70 z-[100] flex items-end justify-center transition-opacity duration-300">
          <div className="bg-white dark:bg-dark-900 rounded-t-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto relative p-6 shadow-2xl">
